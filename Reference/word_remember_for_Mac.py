...
sound_filename = "enableSound.bool"
try:
    with open(sound_filename, "r") as obj:
        contentInFile = obj.read()
    if contentInFile == "true":
        open_sound = True
    if contentInFile == "false":
        open_sound = False
except FileNotFoundError:
    sound = input("Enable Sound? (Y/N)").upper()
    with open(sound_filename, "w+") as obj:
        if sound == "Y":
            open_sound = True
            obj.write("true")
        if sound == "N":
